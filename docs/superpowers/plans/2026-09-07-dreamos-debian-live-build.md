...
- `lb config` invocation: exactly the option set in the spec Section 2, kept in `config/auto/config` as a single call, no drift.
...
| `config/auto/config` | Single `lb config` call with the full option set |
| `config/auto/clean` | Standard `lb clean --purge` wrapper |
...
# live-build generated config subdirs (regenerated by `lb config`)
config/binary
config/bootstrap
config/chroot
config/common
...
**Files:**
- Create: `config/auto/config`
- Create: `config/auto/clean`

**Interfaces:**
- Consumes: the `dreamos-lb` image from Task 2.
- Produces: `config/auto/config` (executable, POSIX `sh`) that runs one `lb config` call with the full spec option set. After it runs, `lb build` (Task 8) knows how to assemble the ISO. `config/auto/clean` (executable) wraps `lb clean --purge`.

- [ ] **Step 1: Create `config/auto/config`**
...
- [ ] **Step 2: Create `config/auto/clean`**
...
Run: `chmod +x config/auto/config config/auto/clean && ls -l config/auto`
...
```
Expected: ends with `P: Considering defaults defined in ...` / no `E:` lines, exit code 0. It populates `config/binary`, `config/bootstrap`, `config/common`, etc. (these are gitignored).

- [ ] **Step 5: Verify the resulting config captured the key options**

Run:
```bash
grep -E 'LB_DISTRIBUTION=|LB_ARCHITECTURES=|LB_BOOTLOADERS=|LB_BINARY_IMAGES=|LB_DEBIAN_INSTALLER=' config/binary config/bootstrap 2>/dev/null
grep 'LB_BOOTAPPEND_LIVE=' config/binary
```
Expected: `LB_DISTRIBUTION="trixie"`, `LB_ARCHITECTURES="amd64"`, `LB_BOOTLOADERS="syslinux,grub-efi"`, `LB_BINARY_IMAGES="iso-hybrid"`, `LB_DEBIAN_INSTALLER="none"`, and `LB_BOOTAPPEND_LIVE` contains `persistence` and `locales=it_IT.UTF-8`.

- [ ] **Step 6: Reset generated config (keep only `auto/`)**

Run: `docker run --rm -v "$PWD:/build" -w /build dreamos-lb ./config/auto/clean; git status --porcelain config/`
Expected: only `config/auto/config` and `config/auto/clean` are untracked; no `config/binary` etc. remain.

- [ ] **Step 7: Update `CHANGES.md`**

```markdown
- Task 3: `config/auto/config` (full `lb config` option set) and `config/auto/clean`.
```

- [ ] **Step 8: Commit**

```bash
git add config/auto/config config/auto/clean CHANGES.md
...
**Interfaces:**
- Consumes: `config/auto/config` from Task 3.
...
docker run --rm --privileged \
    ...
    -w /build \
    "${IMAGE_TAG}" \
    bash -c 'set -euo pipefail; ./config/auto/clean || true; lb config; lb build'
...
docker run --rm --privileged \
    ...
    -w /build \
    "${IMAGE_TAG}" \
    bash -c './config/auto/clean || lb clean --purge'
...
| trixie stable, amd64 | Task 3 `config/auto/config` |
...
- `config/auto/clean` invoked as `./config/auto/clean` in Tasks 3, 7 - consistent, and it is created executable in Task 3 Step 3.
